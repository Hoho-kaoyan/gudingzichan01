...
**初始化脚本会创建：**
- 默认管理员账户（EHR号：0000001，密码：Aa@1234567）
- 常用资产大类（办公用品、电子设备配件等）

**安全提示**：首次登录必须修改密码，新密码必须满足强口令要求（8位以上，包含大写字母、小写字母、数字、特殊字符）。
...
**首次登录提示**：首次登录时会弹出"首次登录必须修改密码"弹窗，请按照强口令要求设置新密码。
...
- 查看浏览器控制台和网络请求
- 确认EHR号和密码正确（默认：0000001 / Aa@1234567）

### Excel导入失败
- 确保Excel文件格式正确
- 检查列名是否与文档中的格式一致（参考 [批量导入指南](批量导入指南.md)）
- 查看后端日志了解具体错误信息
- 确认必填列是否存在
...
- 阅读完整的 [README](../../README.md) 了解详细功能
- 查看 [批量导入教程](批量导入指南.md) 学习如何批量导入数据
- 查看 [功能模块详细说明](../需求与设计/功能模块说明.md) 了解各模块实现
