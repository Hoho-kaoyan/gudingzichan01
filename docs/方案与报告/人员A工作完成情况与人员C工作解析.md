...
**人员C需要完成的工作（拉取新代码前）：**
1. ✅ 创建联动任务类型配置页面（以 Modal 方式完成）
2. ✅ 用户管理增加状态字段和标记离职功能
3. ✅ 组长角色相关前端功能（见第十二章：新代码中已由人员B/D 完成）

**关键依赖：**
- 标记离职接口需要人员B实现（任务13，当前仍未实现）
- 组长权限后端：人员B 已实现 update_asset 校验，get_assets 组长列表过滤仍缺失（见第十二章）
...
### ❌ 待完成的工作（拉取新代码后见第十二章，多数已由 B/D 完成）

#### 1. 任务9：组长角色相关前端 ✅ **已在新代码中完成（人员B/D 已实现）**

**说明：** 拉取新代码后，以下内容已存在，无需人员C重复实现：

1. **`frontend/src/contexts/AuthContext.jsx`** ✅
   - 已导出 `isLeader`、`isAdminOrLeader`

2. **`frontend/src/pages/UserManagement.jsx`** ✅
   - 角色列显示已含组长（roleMap 含 leader）
   - 角色筛选、角色选择框已含「组长」选项

3. **`frontend/src/pages/AssetManagement.jsx`** ✅
   - 已使用 `isLeader`、`isAdminOrLeader` 做权限判断
   - 组长编辑资产时使用人选择框已过滤为本组用户
   - 已对 403 做提示「您只能修改本组资产的使用人」

**后端配合情况：** 人员B 已在 `update_asset` 中实现组长校验（本组资产、本组新使用人）；`get_assets` 尚未按组长过滤列表（组长当前会看到全部资产，若需「组长仅看本组」需人员B 补充）。
...
| 任务9：组长角色相关前端 | ✅ 已完成 | 100% | 新代码中已由人员B/D 实现（含前端+后端 update 校验） |

**总体完成度：** 约97%（任务7 为 Modal 形态 90%，任务8、9 已完成；拉取新代码后见第十二章）

**与人员B的联动状态（拉取新代码后）：**
- 任务8：前端✅ → 仍等待后端❌（人员B任务13 标记离职接口未实现）
- 任务9：前端✅ + 后端 update 校验✅；get_assets 组长列表过滤仍缺（需人员B补充）
...
#### 1. 任务11：场景二情况二 - 调拨联动（reallocation）✅ **已完成（新代码中人员B已实现）**

**实现位置：** `backend/routers/assets.py`、`backend/routers/safety_check_results.py`

**已实现逻辑：**
- `update_asset`：修改使用人时走调拨联动（`is_reallocation`），创建系统任务（source=reallocation）、写入 `PendingReallocation`，不直接更新 `asset.user_id`
- `submit_check_result`：任务完成且 source 为 reallocation 时，查询并处理 `pending_reallocations`，更新资产 `user_id`/`user_group` 并删除 pending 记录

---

#### 2. 任务12：组长权限校验 ⚠️ **部分完成（新代码中人员B已实现 update 端）**

**已实现：** `backend/routers/assets.py` 的 `update_asset`
- 组长只能编辑本组资产（`asset.user_group != current_user.group` 时 403）
- 组长修改使用人时，新使用人必须与本组一致（否则 403）

**未实现：** `get_assets` 接口
- 当前未按组长过滤列表，组长会看到全部资产
- 若产品要求「组长仅能查看本组资产」，需人员B在 `get_assets` 中增加：当 `current_user.role == "leader"` 时，增加条件 `Asset.user_group == current_user.group`
...
| 任务 | 状态 | 完成度 | 备注 |
|------|------|--------|------|
| 任务4：创建系统任务服务 | ✅ 已完成 | 100% | 功能完整 |
| 任务5：入库联动 | ✅ 已完成 | 100% | 单个创建和批量导入都已实现 |
| 任务6：交接联动 | ✅ 已完成 | 90% | 创建申请时已实现 |
| 任务11：调拨联动 | ✅ 已完成 | 100% | 新代码中已实现（assets + safety_check_results） |
| 任务12：组长权限校验 | ⚠️ 部分完成 | 约80% | update_asset 已校验；get_assets 未按组长过滤列表 |
| 任务13：标记离职接口 | ❌ 未完成 | 0% | 需创建 PUT /api/users/{id}/mark-resignation |

**总体完成度：** 约82%（任务11 已完成，任务12 部分完成，任务13 仍缺）
...
**人员C前端：** ✅ 已完成（新代码中已由人员B/D 实现）
- AuthContext 已导出 isLeader、isAdminOrLeader
- 用户管理已含组长角色选项与显示
- 资产管理已实现组长权限控制与 403 提示

**人员B后端：** ⚠️ 部分完成
- ✅ `update_asset` 组长校验已实现（本组资产、本组新使用人）
- ❌ `get_assets` 未按组长过滤，组长目前会看到全部资产（若产品要求「组长仅看本组」需人员B 补充）

**联动状态：** 前端与编辑端已就绪；仅「组长只看本组列表」依赖人员B 在 get_assets 增加过滤。
...
**人员B需要完成：**
- ✅ `update_asset` 组长校验已完成
- ❌ 若产品要求「组长仅看本组资产」：在`get_assets`中当 role 为 leader 时增加 `Asset.user_group == current_user.group` 过滤

**人员C需要完成：**
- ✅ 已在新代码中完成（AuthContext、UserManagement、AssetManagement 组长相关）
...
1. **标记离职功能**
   - 人员C前端 ✅ → 等待人员B后端接口 ❌（任务13）
   - **阻塞状态：** 前端已就绪，等待人员B实现接口后联调

2. **组长权限功能**
   - 人员C前端 ✅、人员B update 校验 ✅；get_assets 组长列表过滤 ❌（按需由人员B补充）
...
1. **人员B实现标记离职接口（任务13）**（优先级最高）
   - 人员C前端已就绪，接口就绪后即可联调

2. **按需：人员B在 get_assets 增加组长过滤**（若产品要求组长仅看本组资产）
   - 前端无需改动

3. **人员C可选：任务7 联动配置改为独立页面**（符合文档约定，非必须）
...
## 十二、拉取新代码后：人员B与人员D已完成内容及人员C仍须完成的工作

（基于当前代码库分析，拉取包含人员B、人员D合入后的最新代码后的状态。）

### 12.1 人员B新完成内容

| 任务 | 状态 | 说明 |
|------|------|------|
| **任务11：调拨联动** | ✅ 已完成 | `assets.py` 中修改使用人时走调拨逻辑（创建任务 + PendingReallocation）；`safety_check_results.py` 中任务完成时处理 pending，更新资产使用人并删除 pending 记录 |
| **任务12：组长权限** | ⚠️ 部分完成 | `update_asset` 已实现：组长只能改本组资产、新使用人须为本组；**未实现**：`get_assets` 未按组长过滤，组长目前会看到全部资产 |
| **任务13：标记离职接口** | ❌ 未完成 | `PUT /api/users/{user_id}/mark-resignation` 仍未实现 |

### 12.2 人员D（或合入）新完成内容（前端）

| 内容 | 状态 | 说明 |
|------|------|------|
| **任务9：组长角色前端** | ✅ 已完成 | `AuthContext.jsx` 已导出 isLeader、isAdminOrLeader；`UserManagement.jsx` 角色列/筛选/表单已含「组长」；`AssetManagement.jsx` 已用 isLeader/isAdminOrLeader，组长使用人过滤为本组，403 提示已存在 |

### 12.3 人员C仍需要完成的工作清单

结合人员C原职责（任务7、8、9）与当前代码状态，**人员C仍需要完成或可选完成**的工作如下：

#### 必须/建议完成（与后端或产品相关）

1. **无必须开发项（仅联调与跟进）**
   - **任务8 标记离职**：前端已就绪，仅需等人员B实现 `PUT /api/users/{user_id}/mark-resignation` 后联调与验收。
   - **任务9 组长**：前端与 update 端校验均已完成；若产品要求「组长只能看本组资产」，需**推动人员B**在 `get_assets` 中增加组长过滤（人员C无后端改动）。

#### 可选完成（提升体验或符合文档约定）

2. **任务7：联动任务类型配置页——改为独立页面（可选）**
   - **当前**：功能已在「安全检查任务管理」页以 Modal 形式实现，功能完整。
   - **可选**：若需符合文档中的「独立配置页」约定，可：
     - 新建 `frontend/src/pages/SafetyCheckAutoConfig.jsx`，将现有 Modal 内容迁入；
     - 在 `App.jsx` 增加路由；
     - 在侧边栏（如仅管理员可见）增加「联动任务配置」入口。
   - **工作量**：约 2–3 小时。不做也不影响现有功能使用。

#### 无需再做的部分

3. **任务9 组长角色前端**：新代码中已由人员B/D 完成，人员C无需重复实现。

### 12.4 人员C待办汇总表

| 序号 | 事项 | 类型 | 说明 |
|------|------|------|------|
| 1 | 标记离职联调 | 联调/验收 | 等人员B实现任务13 后，与后端联调并验收 |
| 2 | 组长仅看本组（若需要） | 推动后端 | 需人员B在 `get_assets` 增加组长过滤；人员C无代码改动 |
| 3 | 联动配置独立页面 | 可选 | 将 Modal 改为独立页+路由+菜单，约 2–3 小时 |

### 12.5 建议优先级

- **优先**：与人员B确认任务13（标记离职接口）排期，接口就绪后尽快联调。
- **按需**：若产品明确「组长只能看本组资产」，与人员B确认在 `get_assets` 增加组长过滤。
- **可选**：若希望与文档完全一致，再安排任务7 独立页面改造。
