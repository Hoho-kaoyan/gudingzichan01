...
## 11. 安全检查类型模块 (safety_check_types.py)
...
- **类型列表**：查看所有检查类型（仅管理员）
- **创建类型**：添加新的检查类型（仅管理员）
- **更新类型**：修改检查类型和检查项（仅管理员）
- **删除类型**：删除检查类型（软删除，仅管理员）
- **检查项配置**：为每个检查类型配置多个检查项，支持必填/可选
...
## 12. 安全检查任务模块 (safety_check_tasks.py)

### 功能说明
管理员发布安全检查任务，任务自动分配给资产使用人。

### 核心功能
- **创建任务**：管理员选择资产和检查类型，发布任务（仅管理员）
- **任务列表**：查看所有任务（管理员）或分配给自己的任务（普通用户）
- **任务详情**：查看任务详情和资产完成情况
- **进度跟踪**：实时显示任务完成进度
- **自动分配**：任务发布后自动分配给资产的使用人
- **任务同步**：资产交接、退回、编辑时自动更新任务分配

### 业务流程
1. 管理员选择检查类型
2. 选择要检查的资产（支持多选）
3. 填写任务标题、描述、截止时间
4. 提交任务，系统自动为每个资产创建任务资产关联
5. 任务分配给资产的使用人
6. 普通用户查看任务并完成检查
7. 管理员查看任务进度
...
### 数据模型
- 任务编号（唯一，自动生成）
- 检查类型ID（关联SafetyCheckType）
- 任务标题
- 任务描述
- 截止时间
- 任务状态（pending/completed/overdue/cancelled）
- 创建人ID
- 完成时间

### 任务资产关联
- 任务ID（关联SafetyCheckTask）
- 资产ID（关联Asset）
- 分配用户ID（关联User，资产的使用人）
- 状态（pending/checked/returned）
- 检查结果（yes/no）
- 检查备注
- 检查项结果（JSON格式）
- 检查时间
...
## 13. 安全检查结果模块 (safety_check_results.py)
...
### 9. 安全检查任务管理页面 (SafetyCheckTaskManagement.jsx) - 管理员
- 任务列表展示
- 发布新任务
- 检查类型管理（集成在任务管理页面）
- 查看任务详情和进度
...
### 10. 我的检查任务页面 (MySafetyCheckTasks.jsx) - 普通用户
- 任务列表展示（待检查/已完成）
- 开始检查功能
- 逐项完成检查
- 提交检查结果
...
### 前端
- 使用React Context管理全局状态（认证信息、交接和审批通知）
- 使用React Router进行路由管理
- 使用Ant Design提供UI组件
- 使用Axios进行HTTP请求，自动添加token
- 支持表格列宽调整
- 侧边栏通知功能（待审批、待确认交接、待检查任务数量）
- 字段名中文化显示（流转记录）
...
### 资产交接时
- 如果资产有未完成的安全检查任务，任务自动更新到新接收人

### 资产退回时
- 如果资产有未完成的安全检查任务，任务状态自动更新为"已退库"
- 已退库的资产不纳入任务进度计算

### 资产编辑时
- 如果修改了使用人，未完成的安全检查任务自动更新到新使用人
- 如果状态改为"库存备用"，未完成的安全检查任务自动标记为"已退库"
