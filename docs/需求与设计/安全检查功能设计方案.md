...
- `check_items_result`：JSON格式存储每个检查项的结果，例如（result 取值为 yes/no）：
  ```json
  [
    {"item": "电源线是否完好", "result": "yes", "comment": "正常"},
    {"item": "设备外观是否正常", "result": "yes", "comment": "正常"},
    {"item": "安全标识是否清晰", "result": "no", "comment": "标识略有磨损"}
  ]
  ```
...
```
frontend/src/pages/
├── SafetyCheckTypeManagement.jsx    # 检查类型管理（管理员，当前未挂路由）
├── SafetyCheckTaskManagement.jsx   # 任务管理（管理员，内含「检查类型管理」弹窗与「联动配置」弹窗）
├── MySafetyCheckTasks.jsx          # 我的检查任务（普通用户）
└── SafetyCheckHistory.jsx          # 检查历史记录（设计稿；当前实现中历史在任务详情/资产维度查看，无独立历史页）
```

**与当前实现一致说明**：检查类型管理在实际产品中通过「安全检查任务」页面内的「检查类型管理」弹窗完成，未使用独立路由；存在 `SafetyCheckTypeManagement.jsx` 组件但未在路由中挂载。检查历史通过任务详情或资产检查历史接口查看，无独立「检查历史」页面。
